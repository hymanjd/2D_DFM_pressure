...
def run_dfnworks(sample_index):
    ...
    cmd = f"python3.11 dfm_driver.py {sample_index} > x{sample_index}.out"
    print(f">> {cmd}")
    ...
    try:
        ...
        jobname = f"single_fracture_pressure_x{sample_index}"
        shutil.rmtree(jobname)
        # Clean up
        os.remove(f'single_fracture_pressure_x{sample_index}.log')
        try:
            os.remove(f'x{sample_index}.out')
